...
def embed_and_store_chunks(
    ...
    for chunk_type, items in grouped_by_type.items():
        ...
        collection_name = chroma_collection_override or f"rag_{chunk_type}"
        ...
        for i in range(0, len(items), BATCH_SIZE):
            ...
            for c in batch:
                ...
                nested_metadata = c.get("metadata") if isinstance(c.get("metadata"), dict) else {}
                metadata = {k: v for k, v in c.items() if k not in {"text", "metadata"}}
    ...
    return {
        ...
        "collection": chroma_collection_override,
    }
