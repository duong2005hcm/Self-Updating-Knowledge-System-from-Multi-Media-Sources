...
def test_ask_route_passes_document_context_scope(monkeypatch):
    service = StubAskService(
        mode="simple",
        answer="document answer",
        contexts=[{"text": "doc ctx"}],
    )
    monkeypatch.setattr(ask_module, "get_ask_service", lambda: service)
    ...
    assert service.prepare_calls[0]["document_id"] == "doc-123"
    assert service.prepare_calls[0]["context_query"] == "AI"
...
def test_ask_service_simple_path_uses_context_service(monkeypatch):
    ...
    class FakeContextService:
        ...
        def retrieve_contexts(
            ...
            *,
            ...
            self.questions.append((question, limit, document_id, context_query))
    ...
    assert fake_context.questions == [("what is this", None, None, None)]
...
def test_ask_service_simple_path_passes_document_scope_to_context_service(monkeypatch):
    captured: dict[str, object] = {}
    ...
    class FakeContextService:
        ...
        def retrieve_contexts(
            ...
            *,
            ...
            return [{"text": "selected doc ctx", "score": 0.9, "metadata": {"document_id": document_id}}]
    ...
    assert _run(service.collect_answer(result.answer_stream)) == "doc answer"
    assert "selected_document_id: doc-99" in captured["question"]
    assert "original_search_query: AI" in captured["question"]
...
def test_ask_service_planner_path_passes_injected_context_service(monkeypatch):
    ...
    prepared = service.prepare_answer(
        ...
        context_query="health",
    )

    assert prepared.mode == "planner"
    assert captured["context_service"] is fake_context
    assert captured["document_id"] == "doc-plan"
    assert captured["context_query"] == "health"
    assert "selected_document_id: doc-plan" in captured["question"]
