...
function LoginBarChart({ buckets }) {
  ...
  const showEvery = n > 24 ? 4 : n > 16 ? 3 : n > 10 ? 2 : 1;
  const showCountEvery = n > 18 ? 2 : 1;
  ...
  return (
    <svg className="admin-chart-svg" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="xMidYMid meet">
      ...
      {(buckets || []).map((b, i) => {
        ...
        const showCount = count > 0 && (i % showCountEvery === 0 || i === n - 1);
        const label = String(b.label || "");
        const shortLabel = label.length > 8 ? `${label.slice(0, 8)}…` : label;

        return (
          <g key={`${label}-${i}-${count}`}>
            <rect x={x} y={y} width={barW} height={bh} rx="6" fill="url(#gLogin)" className="admin-login-bar">
              <animate
                attributeName="height"
                from="0"
                to={String(bh)}
                dur="0.55s"
                begin={`${Math.min(i * 0.03, 0.55)}s`}
                fill="freeze"
              />
              <animate
                attributeName="y"
                from={String(padT + chartH)}
                to={String(y)}
                dur="0.55s"
                begin={`${Math.min(i * 0.03, 0.55)}s`}
                fill="freeze"
