...
export default function Taskbar({
  ...
  tool,
  onSelectTool,
  onOpenAdmin, // Thêm prop này để gọi từ App.jsx
  children,
}) {
    ...
    <div className="taskbar">
      <div className="taskbar-left">
        ...
        <nav className="taskbar-tabs" aria-label="Tools">
          ...
          {/*console.log("Quyền hiện tại của user:", user?.role)*/}
          {/* Nút Quản trị dành riêng cho Admin */}
          {user?.role === 'admin' && (
            <button
              type="button"
              className="taskbar-tab admin-tab-highlight"
              onClick={onOpenAdmin}
              style={{
                color: "#d97706",
                fontWeight: "600",
      ...
      <div className="taskbar-right">
        <div className="taskbar-user">
          ...
          <div className="taskbar-usertext">
            ...
            <div className="taskbar-email">
              {user?.email}
              {user?.role === 'admin' && <span style={{ fontSize: '10px', color: '#d97706', marginLeft: '4px' }}>(Admin)</span>}
            </div>
          </div>
        </div>
