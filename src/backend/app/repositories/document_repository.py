...
class DocumentRepository:
    ...
    def get_documents_by_ids(self, document_ids: list[str]) -> dict[str, Document]:
        ...
        references = [
            self._db.collection(DOCUMENTS_COLLECTION).document(document_id)
            for document_id in normalized_ids
        ]
        snapshots = self._db.get_all(references, **FIRESTORE_CALL_KWARGS)

        items: dict[str, Document] = {}
        for snapshot in snapshots:
            if not snapshot.exists:
                continue
            model = self._snapshot_to_model(snapshot)
