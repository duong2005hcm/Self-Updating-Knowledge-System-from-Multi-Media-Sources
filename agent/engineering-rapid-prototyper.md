...
- Quickly build frontend prototype using React + Ant Design
- Integrate with API or mock API if backend not ready
- Focus on validating user flow
...
```ts
export const mockAsk = async (query) => {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({ answer: "Mock: " + query });
    }, 1000);
  });
};
